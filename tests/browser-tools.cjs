...
async function setup(options = {}, launchOptions = {}) {
  ...
  await page.addInitScript(() => {
    try {
      HTMLCanvasElement.prototype.requestPointerLock = () => Promise.reject(new DOMException('Pointer Lock denied by test fixture', 'NotAllowedError'));
    } catch (_) {}
    const Native = window.AudioContext || window.webkitAudioContext;
    window.__audioEvidence = { contexts: [], sources: 0, peak: 0, energy: 0, samples: 0, frames: [] };
    let previous;
    function frame(time) { if (previous && ['play'].includes(window.__rain?.screen)) window.__audioEvidence.frames.push(time - previous); previous = time; requestAnimationFrame(frame); }
  ...
  return { page, context, browser, errors, failed, requests, consoleErrors, url: process.env.RAIN_URL || `http://127.0.0.1:${server.address().port}/rain-relay/`, close: async () => { await browser.close(); await new Promise(resolve => server.close(resolve)); } };
}
