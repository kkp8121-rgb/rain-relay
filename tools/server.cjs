...
function createServer() {
  return http.createServer((req, res) => {
    let relative;
    try { relative = decodeURIComponent(new URL(req.url, 'http://localhost').pathname).replace(/^\/rain-relay(?=\/|$)/, '').replace(/^\/+/, ''); }
...
if (require.main === module) createServer().listen(Number(process.env.PORT) || 4184, '127.0.0.1', () => console.log('RAIN RELAY local server: http://127.0.0.1:4184/rain-relay/'));
