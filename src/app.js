...
(() => {
  ...
  function showHint() { if (!run || !['edit', 'failed'].includes(run.status)) return; const result = hint(run); if (result && Number.isInteger(result.x)) { hintGhost = result; queueEvents([{ type: 'hint', x: result.x, z: result.z, text: `힌트: ${result.x + 1}, ${result.z + 1} 칸에 ${toolName(result.kind)}이 필요합니다.` }]); } else queueEvents([{ type: 'hint', text: '현재 해답에 맞는 빈 칸이 없습니다.' }]); renderPlay(); }
